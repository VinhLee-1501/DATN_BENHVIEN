...
class PayController extends Controller
{
    ...
    public function order(Request $request)
    {
        ...
        $price = $request->input("total_sale");
        $code = rand(00, 99999);
        if ($request->input('payment_option') == 'vnpay') {
            $vnp_Url = "https://sandbox.vnpayment.vn/paymentv2/vpcpay.html";
            $vnp_Returnurl = "http://127.0.0.1:8000/cua-hang";
            $vnp_TmnCode = "ZAZD6H5N"; //Mã website tại VNPAY 
            $vnp_HashSecret = "VG6VICU7L6V62EHHPKMPR7UG45FBK918"; //Chuỗi bí mật

            $vnp_TxnRef = 41716; //Mã đơn hàng. Trong thực tế Merchant cần insert đơn hàng vào DB và gửi mã này sang VNPAY
            ...
            $amount = $price;
            $orderId = time() . "";
            $redirectUrl = "http://127.0.0.1:8000/cua-hang";
            $ipnUrl = "http://127.0.0.1:8000/cua-hang";
            $extraData = "";
            $requestId = time() . "";
