                ...
                <tbody id="myTable">
                    @foreach($patients as $item)
                        <tr>
                            ...
                            <td class="border-bottom-0"><p class="mb-0 fw-semibold">{{ $item->last_name }} {{ $item->first_name }}</p></td>
                            <td class="border-bottom-0"><p class="badge {{ $item->gender == 1 ? 'bg-info' : 'bg-danger' }} mb-0 fw-semibold">{{ $item->gender == 1 ? 'Nam' : 'Nữ' }}</p></td>
                            <td class="border-bottom-0"><p class="mb-0 fw-semibold">{{ $item->birthday }}</p></td>
                            <td class="border-bottom-0"><p class="mb-0 fw-semibold">{{ $item->Insurance_number }}</p></td>
                            <td class="border-bottom-0"><p class="mb-0 fw-semibold">{{ $item->phone }}</p></td>
                            <td class="border-bottom-0"><p class="mb-0 fw-semibold">{{ $item->emergency_contact }}</p></td>
                            <td class="border-bottom-0 d-flex">
                                <a href="{{ route('system.patients.edit', $item->patient_id) }}" class="btn btn-primary"> <i class="ti ti-pencil"></i></a>
                                <a class="btn btn-warning ms-1" data-bs-toggle="collapse" href="#collapse{{ $item->patient_id }}" role="button" aria-expanded="false" aria-controls="collapse{{ $item->patient_id }}">Chi tiết</a>
                            </td>
                        </tr>
                        <tr id="show">
                            <td colspan="100">
                                                ...
                                                <div class="col-md-6">
                                                    ...
                                                    <div class="d-flex gap-3">
                                                        <div class="col-6">
                                                            ...
                                                            <p><strong>Họ và tên:</strong> {{ $item->last_name . ' ' . $item->first_name }}</p>
                                                            <p><strong>Giới tính:</strong> {{ $item->gender == 1 ? 'Nam' : 'Nữ' }}</p>
                                                            <p><strong>Bảo hiểm y tế:</strong> {{ $item->Insurance_number }}</p>
                                                            <p><strong>SĐT Khẩn cấp:</strong> 0{{ $item->emergency_contact }}</p>
                                                        </div>
                                                        <div class="col-6">
                                                            <p><strong>Ngày sinh:</strong> {{ Carbon\Carbon::parse($item->birthday)->format('d/m/Y') }}</p>
                                                            <p><strong>Địa chỉ:</strong> {{ $item->address }}</p>
                                                            <p><strong>Tạo lúc:</strong> {{ Carbon\Carbon::parse($item->created_at)->format('d/m/Y') }}</p>
                                                            <p><strong>Cập nhật lúc:</strong> {{ Carbon\Carbon::parse($item->updated_at)->format('d/m/Y') }}</p>
                                                    ...
                                                    <table class="table mt-1">
                                                        ...
                                                        <tbody>
                                                            @if(isset($item->medical_id))
                                                                <tr>
                                                                    <td class="py-2"><strong>{{ $item->medical_id }}</strong></td>
                                                                    <td class="py-2"><a href="{{ route('system.detail_medical_record', $item->medical_id) }}">{{ $item->diaginsis }}</a></td>
                                                                </tr>
                                                            @else
                                                                <tr>
